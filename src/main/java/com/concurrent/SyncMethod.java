 ...
 * 2. 先启动一个线程(Thread1), 并让其进入syncMethod对象的synchronized方法(syncMethod1)内, 并使其停在synchronized方法内
 ...
 * 其实此结论是显而易见的.
 * 
 * Created by lfwang on 2017/11/13.
 */
public class SyncMethod {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("thread1 先执行，开始睡眠");
        Thread.sleep(2000); //放弃cpu, 让thread1执行, 以便获的锁
        System.out.println("睡眠结束 执行thread2");
    ...
    public synchronized void syncMethod1() {
        try {
            System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@ (syncMethod1, 已经获取内置锁`SyncMethod.this`)");
            Thread.sleep(10000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@ (syncMethod1, 即将释放内置锁`SyncMethod.this`)");
    }

    public synchronized void syncMethod2() {
        System.out.println("######################## (syncMethod2, 已经获取内置锁`SyncMethod.this`, 并即将退出)");
    }
