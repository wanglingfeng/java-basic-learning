...
public class AVLTree<E> {
     ...
     * 2. 自下向上进行回溯，回溯做两个事情：
     * (1) 其一是修改祖先节点的平衡因子，当插入一个节点时只有根节点到插入节点的路径的节点的平衡因子会被改变，
     * 而且改变的原则是当插入节点在某节点(称为A)的左子树中时，A的平衡因子(称为BF)为BF+1，当插入节点在A的右子树中A的BF-1，
     * 而判断插入节点在左子树中还是右子树只要简单的比较它育A的大小
     * (2) 在改变祖先节点的平衡因子的同时，找到最近一个平衡因子大于2或小于-2的节点，
     * 从这个节点开始调整最小不平衡树进行旋转调整。
     * 由于调整后，最小不平衡子树的高度与插入节点钱的高度相同，故不需要继续调整祖先节点。
     * 这里还有一个热树情况，如果调整BF时，发现某个节点的BF变为0了，则停止向上继续调整，
     ...
     * 如果R的左子树的根结点的BF为1时，做右旋
     * 如果R的左子树的根节点的BF为-1时，先做左旋再右旋
     * 
     * 2. 当R为-2时，即右子树高于左子树：
     * 如果R的左子树的根结点的BF为1时，先做右旋再左旋
     * 如果R的左子树的根节点的BF为-1时，做左旋
     * 
     * @param p
     */
    ...
    private void deleteEntry(Entry<E> p) {
        ...
        Entry<E> replacement = null == p.left ? p.right : p.left;
        
        if (null != replacement) { // 如果其左右子树有其一不为空
            replacement.parent = p.parent;
            if (null == p.parent) { // 如果p为root节点
                root = replacement;
    ...
    private void rotateLeft(Entry<E> p) {
        ...
        Entry<E> r = p.right;
        p.right = r.left; // 把p右子树的左节点嫁接到p的右节点，如注释中，把BL作为A的右子节点
        if (null != r.left) { // 如果B的左节点BL不为空，把BL的副节点设置为A
            r.left.parent = p;
        }
        r.parent = p.parent; // A的父节点设为B的父节点
        if (null == p.parent) { // 如果p是根结点
            root = r; // r变为父节点，即B为父节点
        } else if (p.parent.left == p) { // 如果p是左节点
            p.parent.left = r; // p的父节点左子树为r
        } else { // 如果p是右节点
            p.parent.right = r; // p的父节点右子树为r
        }
    ...
    private void rotateRight(Entry<E> p) {
        ...
        Entry<E> l = p.left;
        p.left = l.right; // 把B的右节点作为A的左节点
        if (null != l.right) { // 如果BR不为null，设置BR的父节点为A
            l.right.parent = p;
        }
        l.parent = p.parent; // A的父节点设为B的父节点
        if (null == p.parent) { // 如果p是根结点
            root = l; // l变为父节点，即B为父节点
        } else if (p.parent.right == p) { // 如果p是右节点
            p.parent.right = l; // p的父节点右子树为l
        } else { // 如果p是左节点
            p.parent.left = l; // p的父节点左子树为l
        }
    ...
    private class BinarySortIterator implements Iterator<E> {
        ...
        public BinarySortIterator() {
            Entry<E> s = root;
            if (null != s) {
                while (null != s.left) { // 找到中序遍历的第一个元素
                    s = s.left;
                }
            }
